const fs = require('fs');
const path = 'client/src/pages/TravelChapterPage.tsx';
...
// We need to replace the entire return <AppLayout>... block down to the end of the page
const layoutStart = content.indexOf('<AppLayout>');
const layoutEnd = content.lastIndexOf('</AppLayout>') + 12;

const newLayout = `
    <AppLayout>
      <div className="max-w-4xl mx-auto h-[calc(100vh-64px)] flex flex-col overflow-hidden bg-[#efeae2]">
        
        {/* Modern WhatsApp-Style Chat Header */}
        <div className="flex items-center px-2 sm:px-4 py-2 sm:py-3 bg-white border-b border-gray-200 shrink-0 z-10 shadow-sm relative">
          <button
            onClick={() => setLocation('/travel-chapters')}
            className="p-2 sm:mr-2 text-gray-500 hover:bg-gray-100 rounded-full transition-colors"
          >
              ...
              <div className="p-6 space-y-8">
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-gray-700 text-sm leading-relaxed whitespace-pre-line">
                    {chapter.description || \`Welcome to the official travel chapter for \${chapter.city}.\`}
                  </p>
        ...
        {/* Chat Messages Area */}
        <div className="flex-1 overflow-y-auto px-2 sm:px-6 py-4 custom-scrollbar flex flex-col gap-1.5 sm:gap-2 relative z-0">
          {!isMember ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center opacity-70">
              <div className="bg-[#008060]/10 p-4 rounded-full mb-3">
                <MessageSquare className="w-8 h-8 text-[#008060]" />
              </div>
              <p className="text-sm text-gray-500 font-medium bg-white/80 px-4 py-2 rounded-lg shadow-sm">No messages yet.<br/>Start the conversation!</p>
            </div>
          ) : (
                  ...
                  <div key={msg.id} className={\`flex w-full \${isMe ? 'justify-end' : 'justify-start'}\`}>
                    <div className={\`flex max-w-[85%] sm:max-w-[75%] gap-2 \${isMe ? 'flex-row-reverse' : 'flex-row'}\`}>
                      ...
                      <div className="flex flex-col relative group/msg">
                        ...
                        {isEditing ? (
                          <div className={\`flex flex-col gap-2 p-2 sm:p-3 shadow-sm w-full min-w-[200px] sm:min-w-[250px] \${
                            isMe 
                              ? 'bg-[#dcf8c6] border border-[#dcf8c6] rounded-md rounded-tr-none' 
                              : 'bg-white border border-gray-200 rounded-md rounded-tl-none'
                          }\`}>
                            <textarea
                              value={editText}
                          ...
                          <div className="relative flex items-center group/bubble">
                            ...
                            <div 
                              className={\`px-3 py-1.5 sm:px-4 sm:py-2 text-[14px] sm:text-[15px] leading-relaxed shadow-sm break-words \${
                                isMe 
                                  ? 'bg-[#dcf8c6] text-gray-900 rounded-md rounded-tr-none' 
                                  : 'bg-white border border-gray-100 text-gray-900 rounded-md rounded-tl-none'
                              }\`}
                            >
                              <div className="flex flex-col">
                                <span>{msg.content}</span>
                                <div className={\`flex items-center justify-end gap-1 mt-0.5 \${isMe ? 'text-gray-500' : 'text-gray-400'}\`}>
        ...
        {/* Chat Input */}
        {isMember && (
          <div className="p-2 sm:p-4 bg-[#f0f2f5] shrink-0 z-10">
...
content = content.substring(0, layoutStart) + newLayout;
fs.writeFileSync(path, content, 'utf8');
console.log('Successfully updated TravelChapterPage.tsx');
