...
interface AppLayoutProps {
  ...
  currentPage?: 'feed' | 'job-portal' | 'events' | 'connections' | 'inbox' | 'profile' | 'settings' | 'forums' | 'alumni-map' | 'blogs' | 'city-chapter' | 'city-chapter-detail' | 'mentorship' | 'podcast' | 'leaderboard' | 'notifications';
}
...
export const AppLayout: React.FC<AppLayoutProps> = ({ children, currentPage = 'feed' }) => {
  ...
  const pageTitles: Record<string, { first: string; second?: string }> = {
    ...
    'city-chapter': { first: 'City', second: 'Chapter' },
    'city-chapter-detail': { first: 'City', second: 'Chapter' },
  ...
  const navItems = [
    ...
    { id: 'city-chapter', icon: Globe, label: 'City Chapter', path: '/city-chapter', section: 'discover', roles: ['alumni', 'student', 'faculty', 'administrator'] },
    ...
    <div className="flex h-screen overflow-hidden bg-gray-50 w-full max-w-full">
      ...
      <div className="flex-1 flex flex-col lg:ml-60 xl:ml-72 min-h-screen overflow-x-hidden max-w-full">
        ...
        <div id="app-main-scroll-container" data-scrollable-container ref={scrollableContainerRef} className={`flex-1 ${activePage === 'inbox' || activePage === 'city-chapter-detail' ? 'overflow-hidden' : 'overflow-y-auto'} overflow-x-hidden mt-14 sm:mt-16 w-full max-w-full`}>
          <div className={`w-full mx-auto ${activePage === 'inbox' || activePage === 'city-chapter-detail' ? 'h-full' : 'max-w-[1400px] xl:max-w-[1600px] 2xl:max-w-[1800px] px-4 sm:px-6 lg:px-8 xl:px-8 py-6 sm:py-8 lg:py-10'}`}>
            {children}
          </div>
        </div>
      </div>
